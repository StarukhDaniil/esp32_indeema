...
static EventGroupHandle_t mqtt_eg;
...
static mqtt_manager_t mqtt_manager;
...
void app_main(void)
{
    ...
    init_mqtt_manager(&mqtt_manager, mqtt_eg);
    ...
    mqtt_manager_start_loop(&mqtt_manager);

    for ( ;; ) {
        vTaskDelay(pdMS_TO_TICKS(1000));
        xEventGroupSetBits(mqtt_eg, MQTT_MANAGER_SEND_STATUS_BIT);
    }
}