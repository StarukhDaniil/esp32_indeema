...
#define ALL_JOYSTICK_BITS JOYSTICK_X0_BIT | JOYSTICK_X4095_BIT | JOYSTICK_Y0_BIT | JOYSTICK_Y4095_BIT | JOYSTICK_SW_PRESSED_BIT
...
static void on_joy_x0_cb(void* joy_reader) {
    xEventGroupClearBits(((joystick_reader_handle_t)joy_reader)->event_group, ALL_JOYSTICK_BITS);
...
static void mh_wifi_handle_pause_cb(void* pvParameters) {
    wifi_manager_handle_t wm = pvParameters;
    xEventGroupSetBits(wm->joy_wifi_eg, WIFI_RECEIVED_PAUSE_BIT);
    uint32_t bits = 0;
    ESP_LOGI("WIFI_MANAGER", "Waiting for resume...");
    while (!(bits & WIFI_RESUME_BIT)) {
    ...
    ESP_LOGI("WIFI_MANAGER", "Resumed!");
    xEventGroupSetBits(wm->joy_wifi_eg, WIFI_RECEIVED_RESUME_BIT);
}
...
void mh_configure_wifi_cbs(wifi_manager_handle_t wm) {
    ...
    wm_add_event(WIFI_PAUSE_BIT, mh_wifi_handle_pause_cb, wm);
}